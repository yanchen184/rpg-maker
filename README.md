...
**runtime 換裝 API**(`packages/engine/src/player.ts`):
...
3. **掛上角色**:`player.setOverlay(manifest, 'outfit', 'char-jacket')`;要進 UI 面板就照 `apps/escape-room/src/main.ts` 的 `slotDefs` 陣列同樣模式加一排按鈕(UI 只列 sheet 已落地的變體)。
...
npm workspaces 三包,引擎與素材管線可被其他遊戲直接吃:

```
packages/engine/          @rpg-maker/engine — 可重用遊戲引擎(直接供 TS 源碼,免 build)
  src/scene.ts            讀場景 JSON,鋪地板/牆、擺物件、碰撞箱、y-sort、線索/裝置/門鎖
  src/player.ts           紙娃娃角色(多層 AnimatedSprite、四向 walk/idle、碰撞滑牆、greet/pickup 動作)
  src/assets.ts           manifest 載入、sheet 切幀、setAssetBase() 注入素材根路徑
  src/editor.ts           場景編輯模式(拖家具、匯出 JSON)
  src/index.ts            barrel export
packages/asset-pipeline/  @rpg-maker/asset-pipeline — codex 生圖管線(不綁專案,吃 RPG_PROJECT_ROOT)
  tools/gen-queue.sh      序列 queue 生圖 + 去背(repo 根的 tools/gen-queue.sh 是相容 wrapper)
  tools/codex_queue.py    常駐 daemon 版排程
  tools/make-*-overlay.py 髮/衣/褲程式化換色、帽子貼位、對齊、壓縮
apps/escape-room/         逃脫遊戲本體(關卡邏輯、HUD、換裝面板、Firebase 多人連線)
assets/                   跨遊戲共用素材庫(manifest、sprite sheets、場景 JSON、prompts)
```
...
1. 開 `apps/parking/`,`package.json` 加 `"@rpg-maker/engine": "*"`(root workspaces 已含 `apps/*`),vite.config 設 `publicDir: '../../assets'`(或自備素材目錄)。
2. 進場先 `setAssetBase(import.meta.env.BASE_URL)`,再 `loadManifest()` → `loadScene('你的場景')` → `buildScene()` → `new Player(...)`。
3. 互動鍵語意由遊戲層決定:引擎不綁 E 鍵,自己在 keydown 分派(要打招呼呼叫 `player.greet()`,撿東西呼叫 `pickupAction()`);載具系統(`Vehicle`)引擎已內建,停車遊戲可直接用。
4. 素材照舊走管線:`RPG_PROJECT_ROOT=<新專案根> packages/asset-pipeline/tools/gen-queue.sh <名稱>`,prompts 放新專案的 `assets/prompts/`。
