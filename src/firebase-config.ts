/**
 * Firebase web 設定(多人連線用,借用尊上現成的 squash-72502 專案的 Realtime Database)。
 *
 ...
 *
 * 這裡只用到 Realtime Database 的 `presence/` 節點做玩家位置廣播,不碰其他遊戲的資料。
 */
 ...
 * 用獨立子路徑 `rpg-maker-presence` 與 squash 自己的遊戲資料隔開,互不干擾;
 * 不需要改動 squash 的 rules、也不需要重新 deploy。
 */
export const PRESENCE_ROOT = 'settings/rpg-maker-presence';
